...
const INPUT_FILE = process.argv[2]; //the IN file path
const OUTPUT_FILE = process.argv[2].replace(/^.*\//, ""); // the out file
...
let html =
  `<html><head><style>body{font-family: monospace;}div{white-space: nowrap}span{display: inline-block}</style></head><body style="margin:0;padding:0">
      ...
      (r) =>
        ...
        r
          .map((c) => {
            const l = c.length;
            return (
              `<span style='color:rgb(${Math.round((255 * l) / max)},0,0);
background-color:rgb(0,${Math.round(256 - (128 * l) / max)},0)'>` +
              ("00" + l.toString()).substr(-3) +
              "</span>"
            );
          })
          .join("") +
...
let solution = Solution.findPerfectSlices(data, layers, minimal[0], minimal[1]);

console.log(`Solution found in ${-time + (time = Date.now())}ms`);
let score = solution.map((s) => (Math.abs(s[2] - s[0]) + 1) * (Math.abs(s[3] - s[1]) + 1)).reduce((a, b) => a + b),
  total = layers.length * layers[0].length;
console.log(`\x1b[32mSolution score: ${score}/${total}\x1b[0m`);

if (!fs.existsSync(`./output`)) fs.mkdirSync(`./output`);
fs.writeFileSync(`./output/${OUTPUT_FILE.replace(/\..*$/, "")}(${score}-of-${total}).out`, solution.length + `\n` + solution.map((s) => s.join(` `)).join(`\n`));
